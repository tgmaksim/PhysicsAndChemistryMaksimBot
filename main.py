...
async def _stop(callback_query: CallbackQuery):
    if await new_callback_query(callback_query, "Остановка функции"): return
    await callback_query.message.edit_reply_markup()
...
def decor_new_callback_query(fun):
    async def new(callback_query: CallbackQuery, state: FSMContext):
        if await new_callback_query(callback_query, "Запуск функций по химии"): return
...
async def _educational_functions(callback_query: CallbackQuery, state: FSMContext):
    if await new_callback_query(callback_query, "Функции по химии"): return
...
async def _educational_data(callback_query: CallbackQuery):
    if await new_callback_query(callback_query, "Запрос информации по предметам"): return
    new = False
